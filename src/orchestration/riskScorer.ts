...
const IMPACT_LEVEL_WEIGHT: Record<string, number> = {
  critical: 1.0,
  high: 0.75,
  medium: 0.4,
  low: 0.15,
 ...
 */
export function deriveComponentFromPath(filePath: string): string | null {
...
function _prettifySegment(segment: string): string {
...
function criticalPathScore(changedFiles: ChangedFile[]): number {
  if (!changedFiles.length) return 0;
  const hits = changedFiles.filter((f) =>
    CRITICAL_PATH_PATTERNS.some((p) => p.test(f.path))
  ).length;
  const ratio = hits / changedFiles.length;
  return clamp(ratio * 100);
}

function blastRadiusScore(affectedTests: AffectedTestCandidate[], changedFiles: ChangedFile[]): number {
  if (!affectedTests.length) return 0;

  const highOrCritical = affectedTests.filter(
    (t) => t.impact_level === "critical" || t.impact_level === "high"
  ).length;
  ...
  const changeSize = changedFiles.reduce(
...
function historicalRiskScore(affectedTests: AffectedTestCandidate[]): number {
  const withHistory = affectedTests.filter(
    (t) => t.failure_rate_7d !== undefined || t.failure_rate_30d !== undefined || t.flakiness_score !== undefined
  );
  if (!withHistory.length) return 0;

  const avgFailureRate7d =
    withHistory.reduce((acc, t) => acc + (t.failure_rate_7d ?? 0), 0) / withHistory.length;
  const avgFlakiness =
    withHistory.reduce((acc, t) => acc + (t.flakiness_score ?? 0), 0) / withHistory.length;
  const recentFailures =
    affectedTests.reduce((acc, t) => acc + (t.recent_failure_count ?? 0), 0);
...
function componentHistoryScore(
  ...
  for (const f of changedFiles) {
    ...
    if (derived) affectedComponents.add(derived.toLowerCase());
  }

  if (!affectedComponents.size) return 0;
  ...
  const relevant = componentHealth.filter((c) => {
    ...
    return (
      affectedComponents.has(name) ||
      // partial match: "Checkout" matches "checkoutservice", etc.
      Array.from(affectedComponents).some(
        (ac) => name.includes(ac) || ac.includes(name)
      )
    );
  });
  ...
  const highRiskCount = relevant.filter(
    (c) => c.risk_level === "HIGH"
  ).length;
 ...
 */
function dependencyBlastScore(blast: DependencyBlast | undefined): number {
  if (!blast || !blast.has_structure) return 0;
  ...
  const blastComponentNames = Object.keys(blast.affected_components ?? {})
    .map((c) => c.toLowerCase());
  const criticalHit = CRITICAL_PATH_PATTERNS.some((p) =>
    blastComponentNames.some((c) => p.test(c)) ||
...
function verificationCoverageScore(affectedTests: AffectedTestCandidate[]): number {
  if (!affectedTests.length) return 50; // unknown — neutral
...
export function computeRiskScore(
  ...
  affectedTests: AffectedTestCandidate[],
  componentHealth: ComponentHealthEntry[] = [],
  dependencyBlast?: DependencyBlast,
): RiskScoreResult {
  ...
  const compExplanation =
    compRaw > 0 && relevantComponents.length > 0
      ? `${relevantComponents.length} affected component(s) have elevated historical failure rates: ${
          relevantComponents
            .slice(0, 3)
  ...
  const blastExplanation = (() => {
    ...
    return (
      `${dependencyBlast.total_expanded} file(s) transitively import changed files ` +
  ...
  const factors: RiskFactor[] = [
    {
      ...
      explanation: `${affectedTests.length} test(s) affected by ${changedFiles.length} changed file(s)${
        affectedTests.filter((t) => t.impact_level === "critical" || t.impact_level === "high").length > 0
  ...
  const merge_signal: "clean" | "review" | "block" =
    risk_level === "BLOCK" ? "block" :
    risk_level === "WARN" ? "review" :
    "clean";

  const merge_recommendation: RiskScoreResult["merge_recommendation"] =
    risk_level === "BLOCK" ? "request_changes" :
  ...
  const confidence = clamp(
    ...
    (dependencyBlast?.has_structure ? 0.05 : 0),
    0, 1
  );
...
export function severityFromRiskLevel(level: RiskLevel): Severity {
