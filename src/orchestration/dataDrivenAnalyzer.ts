...
import { incidentContextFromMetadata } from "./incidentContextAdapter.js";
...
function buildProbableCause(
  ...
  const candidate = impact.affectedTests.find(
    (t) =>
      finding.relatedTestIds.includes(t.test_id ?? -1) ||
  ...
  if (candidate) {
    ...
    if (rate7d !== undefined && rate7d > 0.1) {
      const flakinessNote =
        flakiness !== undefined && flakiness > 0.15
          ? ` with ${pct(flakiness)} flakiness`
          : "";
      parts.push(
        `This test has failed ${pct(rate7d)} of the time in the last 7 days${flakinessNote}.`,
      );
    }
    ...
    if (compLabel) {
      const compHealth = impact.componentHealth?.find(
        (c) =>
          c.component.toLowerCase().includes(compLabel.toLowerCase()) ||
          compLabel.toLowerCase().includes(c.component.toLowerCase()),
      );
      if (compHealth && (compHealth.failure_rate_7d ?? 0) > 0.15) {
        const trendNote =
          compHealth.trend === "worsening" ? " and is trending worse" :
          compHealth.trend === "improving" ? " (trending better)" : "";
        parts.push(
          `The ${compHealth.component} component has a ${pct(compHealth.failure_rate_7d ?? 0)} ` +
    ...
    if (
      candidate.blast_source &&
      candidate.blast_source !== "direct" &&
      candidate.blast_source !== "changed_file"
    ) {
      parts.push(
        `Discovered via transitive import analysis (depth ${candidate.blast_depth ?? "?"})` +
  ...
  if (criticalFiles.length > 0) {
    parts.push(
      `Changed files touch critical paths: ${criticalFiles.slice(0, 3).map((f) => `\`${fileBasename(f)}\``).join(", ")}.`,
    );
  }
...
function buildMergeRecommendation(
  ...
  riskLevel: string,
): ClaudeAnalysis["mergeRecommendation"] {
  const stage = context.runs.tests;
  const blockingCount = context.findings.filter((f) => f.blocking).length;

  if (blockingCount > 0 || stage?.status === "failed") return "request_changes";
  if (stage?.status === "passed" && riskLevel === "BLOCK") return "merge_with_followup";
  if (stage?.status === "passed") return "merge";
  if (riskLevel === "BLOCK") return "request_changes";
  if (riskLevel === "WARN") return "hold";
  return "merge";
}
...
function buildConfidence(
  ...
  let score = 0.55;
  if (context.runs.tests?.status !== undefined) score += 0.25; // execution signal
  if (impact.affectedTests.some((t) => t.failure_rate_7d !== undefined)) score += 0.10;
  if ((impact.componentHealth?.length ?? 0) > 0) score += 0.06;
  if (impact.dependencyBlast?.has_structure) score += 0.04;
  return Math.min(score, 0.96);
}

// ─── summary paragraph ───────────────────────────────────────────────────────

const RECOMMENDATION_LABEL: Record<ClaudeAnalysis["mergeRecommendation"], string> = {
  merge: "Safe to merge.",
...
function buildSummary(
  ...
  riskScore: number,
  riskLevel: string,
  recommendation: ClaudeAnalysis["mergeRecommendation"],
  ...
  if (stage?.status === "passed") {
    ...
    parts.push(
      `${impact.affectedTests.length} test(s) identified across ` +
  ...
  if (criticalFiles.length > 0) {
    parts.push(
      `Changes touch critical paths: ${criticalFiles.slice(0, 2).map((f) => fileBasename(f.path)).join(", ")}.`,
    );
  }

  const highRiskComponents = (impact.componentHealth ?? []).filter(
    (c) => (c.failure_rate_7d ?? 0) >= 0.25 || c.risk_level === "HIGH",
  );
  if (highRiskComponents.length > 0) {
    parts.push(
      `High-risk component(s): ${highRiskComponents
        .slice(0, 2)
  ...
  if (blast?.has_structure && (blast.total_expanded ?? 0) > 0) {
    parts.push(
      `Blast radius: ${blast.total_expanded} file(s) transitively affected ` +
  ...
  if (incident && incident.match_count > 0) {
    parts.push(
      `Engineering memory: ${incident.match_count} similar incident(s) ` +
      `(match score ${incident.incident_match_score}/100).`,
    );
    if (incident.top_resolution?.action) {
      parts.push(
        `Prior fix: "${incident.top_resolution.action.slice(0, 80)}".`,
      );
    }
  }
...
function buildReviewComments(
  context: WorkflowContext,
  impact: ImpactAnalysisResult,
): ClaudeAnalysis["reviewComments"] {
  const comments: ClaudeAnalysis["reviewComments"] = [];

  // Risk factor comments from stored metadata
  const storedFactors = Array.isArray(
    (context.metadata as Record<string, unknown>).risk_factors,
  )
    ? ((context.metadata as Record<string, unknown>).risk_factors as Array<{
        factor: string;
        score: number;
        explanation: string;
      }>)
  ...
  for (const f of storedFactors
    ...
    const severity: Severity =
      f.score >= 70 ? "high" : f.score >= 45 ? "medium" : "low";
    comments.push({
  ...
  const highRiskComponents = (impact.componentHealth ?? []).filter(
    (c) => (c.failure_rate_7d ?? 0) >= 0.30 || c.risk_level === "HIGH",
  );
  if (highRiskComponents.length > 0) {
    comments.push({
      body:
        ...
        highRiskComponents
          ...
          .map(
            (c) =>
              `${c.component} has ${pct(c.failure_rate_7d ?? 0)} 7d failure rate` +
  ...
  if (blast?.has_structure && (blast.total_expanded ?? 0) >= 5) {
    ...
    comments.push({
      body:
        `**Dependency blast radius:** ${blast.total_expanded} file(s) transitively import changed code ` +
...
export class DataDrivenClaudeAnalyzer implements ClaudeAnalysisAdapter {
  async analyze(input: {
    ...
    const incident = incidentContextFromMetadata(context.metadata as Record<string, unknown>);

    const riskScore =
      typeof context.metadata.risk_score === "number"
        ? context.metadata.risk_score
        : 0;
    const riskLevel =
      typeof context.metadata.risk_level === "string"
    ...
    const rootCauses = context.findings.map((finding) => {
      ...
      const memoryMatch = incident?.matches.find(
        (m) =>
          m.match_type === "resolution" &&
    ...
    const warningFindings = context.findings.filter(
      (f) => !f.blocking && f.status === "warning",
    );

    const suggestedFixes: ClaudeAnalysis["suggestedFixes"] = [
      ...blockingFindings.map((f) => {
        const historicalFix =
          incident?.top_resolution?.action ??
    ...
    const riskFactors: ClaudeAnalysis["riskFactors"] = [];
    const storedFactors = Array.isArray(
      (context.metadata as Record<string, unknown>).risk_factors,
    )
      ? ((context.metadata as Record<string, unknown>).risk_factors as Array<{
          factor: string; score: number; weight: number; explanation: string;
        }>)
      : [];
    ...
    if (incident && (incident.incident_match_score ?? 0) > 0) {
      ...
      if (!alreadyHas) {
        riskFactors.push({
          ...
          weight: 0.15,
          explanation:
            `Engineering Memory matched ${incident.match_count} historical signal(s) ` +
