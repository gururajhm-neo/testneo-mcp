...
export type PolicyCode =
  ...
  | "missing_route_map_coverage"
  | "missing_checkout_data_prerequisite";
...
function hasAssertionCoverage(commands: string[] | undefined): boolean {
  if (!commands || !commands.length) return false;
  return commands.some((x) => /verify|assert|expect/i.test(x));
}

function listUnknownNavigateTargets(commands: string[] | undefined, routeMap: Record<string, string>): string[] {
  ...
  for (const cmd of commands) {
    ...
    if (!m?.[1]) continue;
    const raw = m[1].trim();
    if (!raw || raw.startsWith("/") || raw.startsWith("http") || raw.toLowerCase().startsWith("{{base_url}}")) continue;
...
function looksLikeCheckoutFlow(commands: string[] | undefined): boolean {
  if (!commands?.length) return false;
  const t = commands.join("\n").toLowerCase();
  return t.includes("checkout");
}

function hasAddToCart(commands: string[] | undefined): boolean {
  if (!commands?.length) return false;
  return commands.some((x) => /add.*cart|cart.*add/i.test(x));
}

export async function evaluatePreconditionPolicies(
  client: HttpClient,
  ctx: PolicyContext
  ...
  if (!ctx.skip_base_url_check) {
    const base = await evaluateProjectExecutableBase(client, ctx.project_id);
    if (!base.ok) {
      findings.push({
  ...
  if (authRequired) {
    try {
      const envPayload = await client.request<unknown>(
        `/api/web/v1/projects/${encodeURIComponent(String(ctx.project_id))}/environments`
      );
...
export function formatPolicyFailure(result: PolicyResult): Record<string, unknown> {
  return {
    error: "policy_failed",
