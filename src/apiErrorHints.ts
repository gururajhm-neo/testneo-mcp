 ...
 */
export function summarizeTestNeoHttpError(status: number, bodyText: string): string | null {
  ...
  if (root.upgrade_required === true) {
    const parts: string[] = [];
    if (typeof root.error === "string") parts.push(String(root.error));
    if (typeof root.message === "string") parts.push(String(root.message));
    if (typeof root.upgrade_url === "string") parts.push(`Upgrade / pricing path: ${String(root.upgrade_url)}`);
  ...
  if (!isRecord(d)) {
    if (typeof root.error === "string") return String(root.error);
    return null;
  }
  const detail = d;
...
function categorizeHttpStatus(status: number): AgentFacingHttpError["category"] {
...
function defaultNextSteps(status: number, agentSetupUrl?: string): string[] {
  const steps: string[] = [];
  if (status === 401) {
  ...
  if (status === 429) steps.push("Wait and retry with exponential backoff; reduce parallel tool calls.");
  if (status >= 500) steps.push("Retry later; if it persists, contact support with request_id from _telemetry when available.");
  return steps;
}
 ...
 */
export function buildAgentFacingHttpEnvelope(
  status: number,
  path: string,
  bodyText: string,
  opts?: { agentSetupUrl?: string }
