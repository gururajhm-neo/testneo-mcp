...
exports.buildMultiTestRunUiNavigation = buildMultiTestRunUiNavigation;
exports.buildMultiTestRunUiNavigationForClient = buildMultiTestRunUiNavigationForClient;
exports.mergeTestExecutionLinksIntoMultiTestNav = mergeTestExecutionLinksIntoMultiTestNav;
...
function joinWebAppPathWithQuery(appOrigin, pathPrefix, relativePath, query) {
...
function buildMultiTestRunUiNavigation(opts, projectId, testRunId, runId) {
    const apiOrigin = opts.apiOrigin.replace(/\/+$/, "");
    const appOrigin = opts.appOrigin.replace(/\/+$/, "");
    const prefix = opts.appPathPrefix || "";
    const tr = testRunId != null && Number.isFinite(testRunId) && testRunId > 0 ? Math.floor(testRunId) : null;
    return {
        ...
        note: "Open multi_test_runner_url in a browser while logged in to watch batch/API-chain progress and open per-test execution dashboards. " +
            "When TESTNEO_BASE_URL is local (:8001) and TESTNEO_WEB_APP_URL is unset, links use the Vite dev origin (:5173). " +
            "Set TESTNEO_WEB_APP_URL / TESTNEO_WEB_APP_PATH_PREFIX (e.g. /web) if your UI lives elsewhere.",
    };
}
function buildMultiTestRunUiNavigationForClient(client, projectId, testRunId, runId) {
    return buildMultiTestRunUiNavigation({
        apiOrigin: client.getBaseUrl(),
        appOrigin: client.getWebAppBaseUrl(),
        appPathPrefix: client.getWebAppPathPrefix(),
    }, projectId, testRunId, runId);
}
/** Attach per-test execution dashboard URLs from a completed multi-test results payload. */
function mergeTestExecutionLinksIntoMultiTestNav(nav, resultsPayload, opts) {
