...
- **`testneo_developer_release_workflow`** — **Primary E2E entry** (IDE-agnostic). One call: Jira/CSV Engineering Memory → optional `auto_sync_structure` / `generate_if_unmapped` (`generate_engine`: `heuristic` | `langgraph`) → full PR validation (impact, execute, PASS/WARN/BLOCK) → optional Release Bundle + gate. Pass `execution.mode` / `execution.platform` for cloud or local agent. See [CLAUDE_CODE_RELEASE_FLOW.md](./CLAUDE_CODE_RELEASE_FLOW.md).

- **`testneo_sync_code_structure`** — Upload workspace ZIP to TestNeo (`upload-zip-async` + poll). Removes manual structure upload before first PR validate. Requires `confirm=true` + `TESTNEO_MCP_ALLOW_WRITE=true`. Input: `workspace_root` (uses `zip` CLI) or `zip_base64`.

- **`testneo_generate_tests_from_code`** — **VS Code parity** — generate NLP tests from indexed code by scope: `function` | `file` | `folder` | `codebase`. Requires structure sync first. Pass `workspace_root` for function bodies (LangGraph quality). `list_only=true` to discover targets; `confirm=true` to create.

- **`testneo_pr_validation_workflow`** — Release brief only (write-guarded for test execution; planning + risk scoring always run). One-shot Release Brief: runs impact analysis, risk scoring, `DataDrivenClaudeAnalyzer` (inline AI analysis — no external LLM), failure explanations, and prioritised fix plan in a single call. Returns a structured document with risk score, PASS/WARN/BLOCK signal, risk factor breakdown, component health snapshot, per-finding root causes, NOW/NEXT fix plan, and ready-to-paste PR comment draft. Set `confirm: true` + `TESTNEO_MCP_ALLOW_WRITE=true` to also execute impacted tests.
