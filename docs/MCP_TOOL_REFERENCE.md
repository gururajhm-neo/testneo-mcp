...
- **`testneo_developer_release_workflow`** — **Primary E2E entry** (IDE-agnostic). One call: Jira/CSV Engineering Memory → optional `generate_if_unmapped` (auto-create tests for uncovered diff functions) → full PR validation (impact, execute, PASS/WARN/BLOCK) → optional Release Bundle + gate. Pass `execution.mode` / `execution.platform` for cloud or local agent. See [CLAUDE_CODE_RELEASE_FLOW.md](./CLAUDE_CODE_RELEASE_FLOW.md).
